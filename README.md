...
When `tool_policy` is enabled, the Harness pane can also call guarded write tools (`standard.fs.write`, `standard.fs.patch_text`, `standard.fs.apply_unified_patch`) plus `standard.code.container_exec` to create or modify project files and then run Python, Node.js, pnpm, React/Vite, tests, and builds. Existing-file writes require the current sha256 so the model cannot blindly overwrite stale content. Coding or test prompts that look like implementation work are marked with an execution-evidence policy so the OpenAI Responses provider requires a successful code execution tool output before the final answer. Docker remains the default backend: the tool copies the repository into a temporary workspace, runs with network disabled, applies CPU/memory/pid limits, and returns stdout/stderr/exit code as a normal tool call. It does not write changes back to the original repository. Experimental Microsoft eXecution Containers (MXC) support can be enabled with `HARNESSDIFF_CODE_RUNTIME_BACKEND=mxc` and `HARNESSDIFF_MXC_EXPERIMENTAL=1`; `auto` tries MXC only when experimental support is enabled and otherwise falls back to Docker with same-or-stricter policy metadata. MXC results are marked with `preview_warning` and `enforcement_gaps` because the upstream MXC SDK is still an early preview.
